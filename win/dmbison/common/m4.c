...
#define WRITE(f, b, l) ignore_value (_write (f, b, l))
...
/* Process a command line file NAME, and return true only if it was
   stdin.  */
static void
process_file (const char *name, FILE* in)
{
  if (STREQ (name, "-"))
    {
	  ...
	  push_file (in, "extern_stdin", false);
    }
  else
    {
      char *full_name;
